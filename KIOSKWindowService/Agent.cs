...
namespace KIOSKWindowService
{
    class Agent
    {
        ...
        private TcpListener listener = null;//代理端的监听
        ...
        private List<Socket> socketList = new List<Socket>();//存储客户端和目标服务器的连接
        private bool book = false;
        string DataSourceIpOrPortA;//数据源IP和portA
        string DataSourceIpOrPortB;//数据源IP和portB
        ...
        public void dele(object ojs)
        {
            ...
            if (client != null)
            {
                while (true)
                {
                    ...
                    if (client.Connected)
                    {
                        try
                        {
                            bytesReadClient = streamToClient.Read(readClient, 0, readClient.Length); //从流中读出数据并保存在了readClient缓存中
                            string InfoPoint = Encoding.GetEncoding("gb2312").GetString(readClient, 0, bytesReadClient);
                            ...
                            if (reqHeader != null && bytesReadClient < readClient.Length)//如果请求头不为空并且读取的字节小于缓存的长度(请求信息完整)
                            {
                                break;
                            }
                            //如果两空行存在则
            ...
            try
            {
                ...
                if (ipAddressA != null)
                {
                    webClientA.Connect(ipAddressA, Convert.ToInt32(ipA.Substring(ipA.IndexOf(":") + 1, ipA.Length - ipA.IndexOf(":") - 1)));//用指定的ip与web服务器端A建立连接
                }
            }
            catch (Exception)
            {
            ...
            {   
                ...
                if (webClientA != null && webClientA.Connected)
                {
            ...
            while (true)
            {
                ...
                {
                    Socket clients = (Socket)socketList[i];
                    ...
                    try
                    {
                        if (clients == null || !clients.Connected)//如果当前socket2为null或断开连接则跳出for循环
                        {
                            break;
                        }
                    ...
                    if (clients == webClientA.Client)//判断socket2是否为目标服务连接
                    {
